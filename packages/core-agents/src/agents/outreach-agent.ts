  ...
  private async sendEmails(context: AgentContextOrUndefined): Promise<OutreachResult> {
    ...
    return {
      campaigns: recipients.map((recipient, index) => ({
        ...
        status: 'sent',
        recipient,
        subject: emailData.subject || 'Outreach Campaign',
  ...
  private async customizeProposal(context: AgentContextOrUndefined): Promise<OutreachResult> {
    ...
    return {
      campaigns: [{
        ...
        customizations: {
          brandColors: customizations.brandColors || { primary: '#007bff', secondary: '#6c757d' },
          typography: customizations.typography || 'modern',
          layout: customizations.layout || 'standard',
