...
export class BrandVoiceAgent extends AbstractAgent {
  ...
  private async calculateVoiceScore(content: string, brandVoiceId?: string): Promise<number> {
    ...
    // Keyword usage bonus
    const keywordScore = Object.values(analysis.keywordUsage).reduce((sum: number, count: number) => sum + (count as number), 0);
  ...
  async analyzeContentPublic(content: string, contentType?: string, brandVoiceId?: string): Promise<AgentResult> {
    return this.execute({
  ...
  async scoreContentPublic(content: string, brandVoiceId?: string): Promise<AgentResult> {
    return this.execute({
      task: 'score_content',
  ...
  async getSuggestionsPublic(content: string, contentType?: string): Promise<AgentResult> {
    return this.execute({
